<style>
    .lists {
        /*  position: relative; */
        left: 114px;
        position: relative;
        width: 200px;
        height: 240px;
    }

    .item * {
        box-sizing: border-box;
    }

    .item {

        margin: auto;
        box-sizing: border-box;
        display: none;
        z-index: 1;
    }

    .active {
        position: absolute;
        z-index: 33;
        top: 0;
        left: 0;
    }

    .item div img {
        width: 100%;
        height: 220px;
    }

    .item div {
        text-align: center;
    }

    .left,
    .right {
        width: 0;
        border: 20px solid black;
        border-top-color: transparent;
        border-bottom-color: transparent;
    }

    .left {
        border-left-width: 0;
    }

    .right {
        border-right-width: 0;
    }

    .btns {
        width: 360px;
        height: 100px;
        display: flex;
        overflow: hidden;
    }

    .btn img {
        width: 60px;
        height: 80px;

    }

    .btn {
        font-size: 12px;
        text-align: center;
        flex-shrink: 0;
        width: 90px;
        position: relative;
    }

    .controls {
        width: 420px;
        height: 100px;
        position: relative;
        margin-top: 10px;
        display: flex;
        align-items: center;
        justify-content: space-between;
    }
</style>
<div class="half" style="vertical-align:top;">
    <h1>預告片介紹</h1>
    <div class="rb tab" style="width:95%;">
        <div style="width:200px; display:flex; flex-wrap:wrap;">

            <div class="lists">
                <?php
                $posters = $Poster->all(['sh' => 1], " order by rank");
                foreach ($posters as $idx => $poster) {


                    if ($idx == 0) {
                        echo " <div class='item active' data-ani={$poster['ani']}> ";
                    } else {
                        echo " <div class='item' data-ani='{$poster['ani']}'> ";
                    }
                    ?>
                    <!-- <div class="item"> -->
                    <div><img src="./img/<?= $poster['img']; ?>" alt=""></div>
                    <div>
                        <?= $poster['name']; ?>
                    </div>
                </div>
                <?php
                }
                ?>
        </div>
    </div>

    <div class="controls">
        <div class="left"></div>
        <div class="btns">
            <?php
            foreach ($posters as $idx => $poster) {
                ?>
                <div class="btn" data-click="<?=$idx?>">
                    <div><img src="./img/<?= $poster['img']; ?>"></div>
                    <div>
                        <?= $poster['name']; ?>
                    </div>
                </div>
                <?php
            }
            ?>
        </div>
        <div class="right"></div>
    </div>
</div>
</div>
<script>
    $(".item").eq(0).show();

    
    let total = $(".btn").length;
    let anidata = $('.active').data('ani');
    let in_out=0;
    let now = 0;
    let timer = setInterval("slide()", 5000);
    let clickON=0;
    let putIn=1000;
    let putOut=2000;

$('.btn').click(function(){
    clickON=1;
    let click1=($(this).data('click'));
    now=click1;
    // timer =setInterval("slide()", 5000);
})


    function slide() {

        if(clickON==1){
            clickON=0;
            console.log('clicked');
            return ;
        }

        $(".item").attr("class", "item");
        // $(".item").fadeOut(500);
        // $(".item").hide(1000);
        ani1(anidata,0);
        
        now = nowADD(now,total);
        $(".item").eq(now).attr("class", "item active");

        console.log("normal=",anidata);

        anidata = $('.active').data('ani');

        console.log("active=",anidata);
        // $(".item").hide(1000);
        // $(".item").eq(now).show(1000);
        // $(".item").eq(now).fadeIn(1000);
        ani1(anidata,1);
    }
  

    function nowADD(now,total) {
        now = now + 1;
        if (now >= total) {
            now = 0
        }
        return now;
    }

    function ani1(anidata, in_out) {
    
        switch (anidata) {
            case 1:
                if (in_out == 0) {
                    $(".item").fadeOut(putIn);
                } else {
                    $(".item").eq(now).fadeIn(putOut);
                    $(".active").fadeIn(putOut);
                }
                break;
            case 2:
                if (in_out == 0) { $(".item").hide(putIn); } else { $(".active").show(putOut); }
                break;
            case 3:
                if (in_out == 0) { $(".item").slideUp(putIn); } else { $(".active").slideDown(putOut); }
                break;
        }

    }










    let p = 0;
    $(".left,.right").on("click", function () {
        console.log(p);
        console.log(total);
        let arrow = $(this).attr('class');

        switch (arrow) {
            case "right":
                if (p + 1 <= (total - 4)) {
                    p = p + 1;

                }
                break;
            case "left":
                if (p - 1 > 0) {

                    p = p - 1;
                }
                break;
        }

        $(".btn").animate({ right: 90 * p });

    })


</script>

<style>
    .movies {
        display: flex;
        flex-wrap: wrap;
    }

    .movie {
        display: flex;
        flex-wrap: wrap;
        box-sizing: border-box;
        padding: 2px;
        margin: 0.5%;
        border: 1px solid #ccc;
        border-radius: 5px;
        width: 49%;
    }
</style>
<div class="half">
    <h1>院線片清單</h1>
    <div class="rb tab" style="width:95%;">
        <div class="movies">
            <?php
            $today = date("Y-m-d");
            $ondate = date("Y-m-d", strtotime("-2 days"));
            $total = $Movie->count(" where `ondate`>='$ondate'  && `ondate` <='$today'  && `sh`=1");
            $div = 4;
            $pages = ceil($total / $div);
            $now = $_GET['p'] ?? 1;
            $start = ($now - 1) * $div;
            $movies = $Movie->all(" where `ondate`>='$ondate'  && `ondate` <='$today'  && `sh`=1 order by rank limit $start,$div");
            foreach ($movies as $movie) {
                ?>
                <div class="movie">
                    <div style="width:35%">
                        <a href='?do=intro&id=<?= $movie['id']; ?>'>
                            <img src="./img/<?= $movie['poster']; ?>" style="width:60px;border:3px solid white">
                        </a>
                    </div>
                    <div style="width:65%">
                        <div>
                            <?= $movie['name']; ?>
                        </div>
                        <div style="font-size:13px;">分級: <img src="./icon/03C0<?= $movie['level']; ?>.png"
                                style="width:20px">
                        </div>
                        <div style="font-size:13px;">上映日期:
                            <?= $movie['ondate']; ?>
                        </div>
                    </div>
                    <div style="width:100%">
                        <button onclick="location.href='?do=intro&id=<?= $movie['id']; ?>'">劇情介紹</button>
                        <button onclick="location.href='?do=order&id=<?= $movie['id']; ?>'">線上訂票</button>
                    </div>
                </div>
                <?php
            }
            ?>
        </div>
        <div class="ct">
            <?php
            if ($now - 1 > 0) {
                $prev = $now - 1;
                echo "<a href='?p=$prev'> < </a>";
            }

            for ($i = 1; $i <= $pages; $i++) {
                echo "<a href='?p=$i'> $i </a>";
            }

            if ($now + 1 <= $pages) {
                $next = $now + 1;
                echo "<a href='?p=$next'> > </a>";
            }
            ?>

        </div>
    </div>
</div>